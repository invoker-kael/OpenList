...
func admin(g *gin.RouterGroup) {
	...
	webdavWriteback.GET("/history", handles.WebDAVWritebackHistoryList)
	webdavWriteback.GET("/history/summary", handles.WebDAVWritebackHistorySummary)
	webdavWriteback.POST("/history/cleanup", handles.WebDAVWritebackHistoryCleanup)
	webdavWriteback.GET("/settings", handles.WebDAVWritebackSettings)
