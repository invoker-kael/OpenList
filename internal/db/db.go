...
func Init(d *gorm.DB) {
	db = d
	err := AutoMigrate(new(model.Storage), new(model.User), new(model.Meta), new(model.SettingItem), new(model.SearchNode), new(model.TaskItem), new(model.SSHPublicKey), new(model.SharingDB), new(model.WebDAVWritebackObject), new(model.WebDAVWritebackHistory), new(model.WebDAVWritebackReceiveFence), new(model.WebDAVWritebackAdmissionFence), new(model.WebDAVWritebackReceiveReservation), new(model.WebDAVProviderOperation))
	if err != nil {
