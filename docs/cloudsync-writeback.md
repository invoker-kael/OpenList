...
A **single** 115 absence, type mismatch, size mismatch or SHA-1 mismatch is no longer allowed to evict a completed file's canonical identity. OpenList records the first divergent provider observation in the existing completed-row verification fields and keeps returning the NAS-authoritative size/mtime/ETag to Cloud Sync. After the configured verification interval, a normal cached directory listing still cannot evict the row: OpenList performs a separate `Refresh: true` parent listing and only that fresh view may confirm the divergence. Repeated PROPFIND scans over the same stale cache therefore cannot manufacture a second observation. An exact provider match clears the pending divergence evidence, while incomplete fresh 115 metadata remains inconclusive and keeps the canonical identity. This specifically prevents stale list cache or transient post-upload NotFound from turning into another PUT and then an upload loop.
