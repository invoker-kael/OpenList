...
- Fresh provider directory probes have their own budget. With `provider_probe_workers=2`, at most two distinct parent refreshes run at once; same-parent checks still collapse through single-flight before consuming a probe slot. WebDAV snapshot misses, background VERIFY/MKCOL/DELETE refreshes, and direct completed-object health reconciliation all consume this same budget only when they become the fresh-list leader. Setting the value to 0 or to a value greater than or equal to `workers` disables the separate probe cap.
