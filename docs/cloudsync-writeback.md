...
- Provider MOVE metadata reconciliation locks source and destination subtrees in one transaction. Exact path collisions merge into the destination generation; old source rows remain tombstones when needed to invalidate in-flight workers. Destination-only rows with unknown source correspondence enter a fresh reconciliation grace instead of being deleted blindly, preventing provider-only children from being accidentally removed after the move.
...
- For `Overwrite: T`, the provider destination is removed and force-refreshed before the source is moved. If 115 still exposes the old destination, WebDAV returns 503 and leaves the source untouched so Cloud Sync can retry after the provider consistency window. Canonical COPY/MOVE reconciliation is only applied after HTTP 201/204 provider success; 403/409/412/503 responses leave the existing MySQL view unchanged.
...
If 115 visibility is slower than the configured verification window, a verifying generation is eventually re-queued. Before uploading the payload again, retry workers perform one fresh remote verification. If the correct size/SHA-1 has appeared in 115 during the backoff, the generation is completed from that evidence without sending the same encrypted payload a second time. The same rule now covers process/container restarts: rows left in `UPLOADING` are recovered as retries, forcing an exact 115 size/SHA-1 check before retransmission. If the provider PUT succeeded but the following MySQL transition to `VERIFYING` fails transiently, OpenList also best-effort re-queues that generation as a verification-first retry instead of leaving it stranded in `UPLOADING`. This specifically reduces duplicate large encrypted uploads while retaining the same integrity gate.
