...
> **Operations:** For day-to-day use, start with the [English operations guide](./cloudsync-writeback-operations.md) or the [Chinese operations guide](./cloudsync-writeback-operations.zh-CN.md). They focus on the user-facing states, recovery actions, and the few cases that actually need Cloud Sync intervention.
...
- Upload verification and completed-object health probing use separate cadences. `verify_interval_seconds` remains the short convergence interval for a generation that is still uploading/verifying, while a healthy completed object whose local spool has already been released reuses its last generation-scoped remote evidence for `completed_remote_probe_seconds` (1800 seconds by default) before background health reconciliation asks the provider again. A suspected divergence still uses the short separated-confirmation window. 115 parent refreshes used by WebDAV snapshot misses, VERIFY, and completed-object health reconciliation now share the same fresh-only per-parent single-flight, so those paths cannot issue parallel `Refresh: true` LIST calls for the same directory. VERIFY still skips the per-file `GET`: one fresh directory listing supplies size/SHA-1 for the trigger and can complete up to 64 already-`VERIFYING` siblings whose canonical size/SHA-1 match that same snapshot. The same fresh LIST can refresh stale matching `COMPLETED` sibling evidence with one generation-guarded batch UPDATE instead of one UPDATE per file. In addition, the single-flight leader that actually executes a successful `Refresh: true` parent LIST opportunistically scans at most 64 healthy `COMPLETED` no-spool rows for that parent and refreshes their `remote_object_id`, `remote_sha1`, `remote_generation`, and `remote_verified_at`. A provider-snapshot cache hit performs no verification SQL, and waiters that merely join the same fresh LIST do not repeat the opportunistic scan. Rows with active divergence/retry evidence, missing canonical SHA-1, missing provider SHA-1, size mismatch, or SHA-1 mismatch are excluded from that healthy piggyback path. Every evidence UPDATE remains generation/state guarded, so a concurrent PUT/MOVE/DELETE cannot attach old provider evidence to a newer canonical generation. Missing or mismatched VERIFYING siblings remain on the existing separated divergence/retry path.
...
- Long-running receives renew their durable MySQL receive lease on an independent 10-second heartbeat. The receive lease is one minute, so a live slow PUT remains protected while crash residue is released quickly instead of blocking the same path for a long period.
...
- DELETE, MKCOL, PROPPATCH, COPY and MOVE now consult the durable receive fence before mutating a path; tree mutations also check active descendant receives. Tree checks fetch only live receive rows through a composite `active_receivers + receive_lease_until` index and apply strict descendant boundaries in memory instead of running recursive TEXT `LIKE + COUNT` scans. A transient overlap returns 503 + `Retry-After`, while the generation/mutation fences remain the final correctness backstop for races between the check and the mutation. Successful canonical PUT, DELETE, MKCOL and canonical-fast-path COPY/MOVE mutations also invalidate the affected in-memory provider namespace snapshots. Invalidation is passive: it never performs a provider LIST by itself; it only prevents the next ordinary directory revalidation from trusting a pre-mutation namespace snapshot. Each parent carries an in-memory invalidation revision, so a `Refresh: true` LIST that started before a concurrent mutation may finish and serve its current canonical-overlay request, but it is not allowed to repopulate the cache after that mutation invalidated the parent.
...
- Fresh provider directory probes have their own budget. With `provider_probe_workers=2`, at most two distinct parent refreshes run at once; same-parent checks still collapse through single-flight before consuming a probe slot. WebDAV snapshot misses and background VERIFY/MKCOL/DELETE refreshes acquire this budget inside the fresh-list leader, while the completed-canonical maintenance loop reserves the same budget around `ReconcileDirect`. Setting the value to 0 or to a value greater than or equal to `workers` disables the separate probe cap.
