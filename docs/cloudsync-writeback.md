...
A provider COPY that returns an ambiguous error is persisted as `FAILED`. If the destination nevertheless matches the captured file/tree identity it is recovered as success; if it is absent or partial, OpenList removes the partial destination, confirms 115 absence across the short consistency fence, retires the intent, and lets Cloud Sync retry cleanly. Pre-mutation 403/404/409/412/503 responses retire the intent immediately because the provider mutation did not begin. MOVE keeps its stricter ambiguous-error behavior because its source may already have been relocated.
