...
- For `Overwrite: T`, the provider destination is removed and force-refreshed before the source is moved. If 115 still exposes the old destination, WebDAV returns 503 and leaves the source untouched so Cloud Sync can retry after the provider consistency window.
