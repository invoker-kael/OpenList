...
if [ "$1" = "dev" ]; then
  ...
  if [ -n "$OPENLIST_FRONTEND_REF" ]; then
    webVersion="$OPENLIST_FRONTEND_REF"
  else
...
if [ "$buildType" = "dev" ]; then
  ...
  if [ "$OPENLIST_FRONTEND_PREBUILT" = "1" ]; then
    echo "using prebuilt frontend from $frontendRepo@$OPENLIST_FRONTEND_REF"
